...
用到 `WebHooks` ，每次提交或其他操作时候，会访问设置的URL

![image-20211112124946135](https://cdn.jsdelivr.net/gh/oddfar/static/PicGo/20211112125800.png)

设置密码，防止他人恶意提交

![image-20211112125023065](https://cdn.jsdelivr.net/gh/oddfar/static/PicGo/20211112125806.png)
...
//是否等于Gitee设置的秘钥
if($headers['X-Gitee-Token'] == "123456789"){
    //调用
    echo "ok";
    //执行脚本
    shell_exec('sh /www/wwwroot/note.oddfar.com/deploy.sh');
}else{
    echo "no";
}
...
1、宝塔里面的设置，默认禁止 `shell_exec` 函数，需要自己手动打开

2、注意权限，我的网站执行命令权限是 www，需要把涉及到的文件所有者改成 www

![image-20211112125524977](https://cdn.jsdelivr.net/gh/oddfar/static/PicGo/20211112125718.png)
